...
$nuevafecha = date ( 'm/j/Y' , $nuevafecha );
	$track = tracking($_POST['receipt'], "01/01/2000", $fecha);
if (isset($track[0]['ID'])){
...
$ultimos3 = substr($tr[0]["Box"],-3); 
		foreach ($tr as $t) {
			if (isset($t["Box"])){
				if (substr($t["Box"],-3) == $ultimos3){
					...
					if ($existe->num_rows == 0) {
						insertaTracking($t['ID'], $t['Box'], $t['Date'], $t['Time'], $t['Note'], $t['Status'], $t['Operator'], $t['Tracking'], $_POST['receipt']);
	...
	$result = buscaTracking($_POST['receipt']);
...
<br>
<div align="center">
<form name='myForm' method="post" action="rastreo.php">
<table>
<tr>
	<td>Receipt:</td>
	<td><input type='text' id="receipt" name="receipt" class="form form-control" /></td>
    <td><input type="submit" id="Buscar" name="Buscar" onclick='ajaxFunction()' class="btn btn-success" value='Buscar'/></td>
</tr>
</table>
</form>
</div>
