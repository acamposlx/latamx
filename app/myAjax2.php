...
<?php
    	...
    	while($row = $result->fetch_assoc()) {
			if ($row["type"] != 'Hold'){
				//busqueda tracking
				$track = tracking($row["receipt"], "01/01/2000", "02/28/2017");
				foreach($track as $tr){
					foreach ($tr as $t) {
						$bt = buscaTracking_ID2($t['ID']);
						if($t['Status']=='ENTREGADO!'){
							entrega_receipt2($row["receipt"]);	
							//echo "El embarque " . $row["receipt"] . "fue actualizado, por favor refresque la página para validar la información";
						}
						if ($bt==0){
							insertaTracking2($t['ID'], $t['Box'], $t['Date'], $t['Time'], $t['Note'], $t['Status'], $t['Operator'], $t['Tracking'], $row["receipt"]);
						}
					}
				}
